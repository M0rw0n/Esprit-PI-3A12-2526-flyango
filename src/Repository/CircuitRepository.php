...
class CircuitRepository extends ServiceEntityRepository
{
    ...
    public function searchVisible(?string $q, ?string $difficulte, string $sort = 'recent', ?User $user = null, ?string $destination = null, ?string $source = null): array
    {
        ...
        if ($destination) {
            $qb->andWhere('c.destination LIKE :destination')->setParameter('destination', '%' . $destination . '%');
        }
    ...
    public function getTopCircuitsByFavorites(int $limit = 6): array
    {
        return $this->createQueryBuilder('c')
            ->select('c, COUNT(f.id) as HIDDEN favCount')
            ->leftJoin('App\Entity\FavoriteCircuit', 'f', 'WITH', 'f.circuit = c')
            ->andWhere('c.actif = :actif')
            ->andWhere('c.sourceType = :source')
            ->setParameter('actif', true)
            ->setParameter('source', 'admin')
