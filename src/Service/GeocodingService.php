...
class GeocodingService
{
    ...
    public function __construct(
        ...
        $this->apiKey = $geoapifyApiKey ?: $_ENV['APP_GEOAPIFY_API_KEY'] ?? $_SERVER['APP_GEOAPIFY_API_KEY'] ?? '';
        $this->requestStack = $requestStack;
        error_log('GeocodingService init with key: ' . ($this->apiKey ? substr($this->apiKey, 0, 8) . '...' : 'EMPTY'));
    }
    ...
    public function getCoordinates(string $address): ?array
    {
        ...
        if (!$response) {
            error_log('Geocoding: Nominatim request failed for: ' . $address);
            return null;
        }
