...
private function getTopRatedCircuits(CircuitRepository $circRepo): array {
    ...
    private function fillHebergement(Hebergement $h, Request $request): void
    {
        ...
        // Handle 360° photos
        $photos360 = $h->getPhotos360() ?: [];
        $files360 = $request->files->get('photos_360');
        
        if ($files360 && is_array($files360)) {
            $dir = $this->getParameter('kernel.project_dir') . '/public/uploads/360';
            if (!is_dir($dir)) mkdir($dir, 0777, true);
            
            foreach ($files360 as $file360) {
                if ($file360 && $file360->getError() === UPLOAD_ERR_OK) {
                    $ext = strtolower(pathinfo($file360->getClientOriginalName(), PATHINFO_EXTENSION));
                    if (in_array($ext, ['jpg', 'jpeg', 'png'])) {
                        $filename = '360_' . uniqid() . '.' . $ext;
                        $file360->move($dir, $filename);
                        $photos360[] = '/uploads/360/' . $filename;
                    }
                }
            }
        }
        
        // Handle 360 photo deletion
        $photos360ToDelete = $request->request->get('photos360_to_delete');
        if ($photos360ToDelete) {
            $toDelete360 = array_filter(array_map('trim', explode(',', $photos360ToDelete)));
            $projectDir = $this->getParameter('kernel.project_dir');
            foreach ($toDelete360 as $p) {
                $path = $projectDir . '/public' . $p;
                if (file_exists($path)) @unlink($path);
            }
            $photos360 = array_diff($photos360, $toDelete360);
        }
        
        if (!empty($photos360)) {
            $h->setPhotos360($photos360);
        }
    }
...
PROMPT;
    ...
    public function geocode(Request $request, GeocodingService $geocodingService): JsonResponse
    {
        ...
        if (!$coords) {
            $coords = $geocodingService->getCoordinates($address);
        }
