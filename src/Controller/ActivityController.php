...
<<<<<<< HEAD
        ...
        private readonly ?ContentModerationService $moderationService = null,
    ) {
    }
...
<<<<<<< HEAD
    ...
    public function show(int $id, ActivityRepository $repo, ReviewRepository $reviewRepo): Response
    {
        $a = $repo->find($id);
        if (!$a) throw $this->createNotFoundException();

        $reviews = $reviewRepo->findByActivityId($id, 50);
        $reviewCount = $reviewRepo->countByActivityId($id);
        $sentimentSummary = $reviewRepo->getSentimentSummaryByActivityId($id);
        $positive = $negative = $neutral = 0;
        foreach ($sentimentSummary as $row) {
            if (in_array($row['label'], ['excellent', 'good', 'positive'])) $positive += $row['count'];
            elseif (in_array($row['label'], ['negative', 'bad'])) $negative += $row['count'];
