...
class HomeController extends AbstractController
{
    ...
    public function index(
        ...
        $postCommentCounts = $forumRepo->findCommentCountsByPostIds($postIds);
        $villesData = $hebRepo->getDistinctVilles();
        $villes = array_values(array_filter(array_column($villesData, 'ville')));

        $avisCounts = [];
        if (!empty($hebergements)) {
            $counts = $hebRepo->getAvisCountsByHebergementIds(array_map(fn($h) => $h->getId(), $hebergements));
            foreach ($counts as $c) {
                $avisCounts[(int) $c['hebergement_id']] = (int) $c['nb_avis'];
            }
        }
