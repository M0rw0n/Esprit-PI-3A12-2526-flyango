...
<<<<<<< HEAD
    public function applyPromo(Request $request, EntityManagerInterface $em): JsonResponse
    {
        try {
            ...
            $nights = (int) $request->request->get('nights', 1);
            ...
            $promo = $conn->fetchAssociative(
                "SELECT * FROM promo_code WHERE code = ? AND active = 1",
                [$upperCode]
            );

            if (!$promo) {
                return new JsonResponse(['success' => false, 'message' => 'Code promo invalide ou inactive'], 400);
            }

            if ($promo['valid_from'] && new \DateTime($promo['valid_from']) > new \DateTime()) {
                return new JsonResponse(['success' => false, 'message' => 'Code promo pas encore valide'], 400);
            }
            if ($promo['valid_until'] && new \DateTime($promo['valid_until']) < new \DateTime()) {
                return new JsonResponse(['success' => false, 'message' => 'Code promo expiré'], 400);
        }
        if ($promo['max_uses'] && $promo['used_count'] >= $promo['max_uses']) {
            return new JsonResponse(['success' => false, 'message' => 'Code promo épuisé'], 400);
        }
        if ($promo['min_nuits'] && $nights < $promo['min_nuits']) {
            return new JsonResponse(['success' => false, 'message' => 'Minimum ' . $promo['min_nuits'] . ' nuits requis'], 400);
        }
        if ($promo['min_amount'] && $originalPrice < $promo['min_amount']) {
            return new JsonResponse(['success' => false, 'message' => 'Minimum ' . $promo['min_amount'] . ' TND requis'], 400);
        }
